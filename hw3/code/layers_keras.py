...
class Conv2D(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.kernel = tf.Variable(
            self.kernel_initializer((self.kernel_size) + (input_shape[-1], self.filters)), trainable=True)
        self.bias = tf.Variable(self.bias_initializer(self.filters), trainable=True)
        return None
    ...
    def call(self, inputs, training=False):
        ...
        outputs = tf.nn.convolution(
            inputs, self.kernel, self.strides, self.padding)
...
class BatchNormalization(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        axis_dim = input_shape.dims[self.axis].value

        self.beta = tf.Variable(self.beta_initializer(1) * self.center, True)

        self.gamma = tf.Variable(self.gamma_initializer(1) * self.scale, True)

        self.moving_mean = tf.Variable(
            self.moving_mean_initializer(axis_dim), False)

        self.moving_variance = tf.Variable(
            self.moving_variance_initializer(axis_dim), False)
    ...
    def call(self, inputs, training=False):
        ...
        if training:
            ...
            mask = tf.random.uniform(
                shape=self.noise_shape if self.noise_shape != None else tf.shape(inputs), minval=0, 
                maxval=1, seed=self.seed) >= self.rate
                
            return tf.cast(mask, dtype=tf.float32) * inputs / (1.0 - self.rate)
        else:
            return inputs
